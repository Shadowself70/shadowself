  ...
  async fetch(request, env) {
    ...
    try {
      if (request.method === "GET" && url.pathname === "/health") {
        return json({ ok: true, service: "assteroids-scoreboard" }, 200, corsHeaders);
      }

      if (request.method === "GET" && url.pathname === "/assteroids/leaderboard") {
        return await handleGetLeaderboard(env, corsHeaders, url);
      }

      if (request.method === "POST" && url.pathname === "/assteroids/score") {
...
async function handleSubmitScore(request, env, corsHeaders) {
  ...
  const score = clampNumber(payload?.score, 0, 99999999, null);
  const playerName = sanitizePlayerName(payload?.playerName);

  if (score === null) {
    return json({ error: "Score must be a valid number." }, 400, corsHeaders);
  }

  await env.DB.prepare(
    `
      INSERT INTO assteroids_scores (player_name, score)
      VALUES (?, ?)
    `,
  )
